...
function createSession(app: Express, token: string, resumeId: string) {
  return request(app)
...
describe("AI rate limit shared Redis E2E", () => {
  ...
  it("shares RATE_LIMIT_AI_MAX count across two Express app instances", async () => {
    ...
    try {
      ...
      await createSession(app1, token, resume.id).expect(201);
      await createSession(app1, token, resume.id).expect(201);
      await createSession(app2, token, resume.id).expect(201);

      const responseOnApp1 = await createSession(app1, token, resume.id);
      expect(responseOnApp1.status).toBe(429);
      expect(responseOnApp1.body).toEqual({
        message: "Too many requests, please try again later.",
      });

      const responseOnApp2 = await createSession(app2, token, resume.id);
